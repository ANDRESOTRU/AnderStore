...
struct ProfilesListView: View {
    ...
    var body: some View {
        ...
        .alert("Purge All Profiles?", isPresented: $showPurgeAllConfirmation) {
            ...
            Text("This will permanently delete all \(viewModel.profiles.count) provisioning profile(s) for team '\(viewModel.team?.name ?? "")' on Apple's developer portal. AnderStore will automatically generate fresh profiles on next app install or refresh.")
        }
