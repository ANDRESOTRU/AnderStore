...
final class SourcesViewController: UICollectionViewController
{
    ...
    override func viewDidLoad()
    {
        ...
        self.placeholderView.detailTextLabel.text = NSLocalizedString("Sources determine what apps are available in AnderStore. The more sources you add, the better your AnderStore experience will be.\n\nDon’t know where to start? Try adding one of our Recommended Sources!", comment: "")
            ...
            func finish(_ result: Result<Void, Error>)
            {
                DispatchQueue.main.async {
                    ...
                    case .failure(var error as SourceError):
                        let title = String(format: NSLocalizedString("“%@” could not be added to AnderStore.", comment: ""), error.$source.name)
